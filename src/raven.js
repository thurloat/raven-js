...
//
// Requires:
//     * reqwest (included in the full and minified distribution files)
...
(function(){
    ...
    Raven.config = function(config) {
        ...
        Raven.util.each(config, function(key, option) {
            self.options[key] = option;
        });

        // Expand server base URLs into API URLs
        Raven.util.each(self.options.servers, function(i, server) {
    ...
    Raven.getHeaders = function(callback) {
        var headers = {};
        ...
        if (self.options.fetchHeaders && !self.options.testMode) {
            var req = reqwest({
              ...
              complete: function(resp) {
                headers = req.request.getAllResponseHeaders(); 
    ...
    Raven.parseHeaders = function(headers_string) {
         ...
         */
        var headers = {};
        Raven.util.each(headers_string.split('\n'), function(i, header) {
    ...
    Raven.captureException = function(e, options, complete) {
        ...
        self.process(e, fileurl, lineno, traceback, options, complete);
    };

    Raven.captureMessage = function(msg, options, complete) {
        var data = self.arrayMerge({
            'message': msg
        }, options);

        self.send(data, complete);
    };

    Raven.process = function(message, fileurl, lineno, traceback, options, complete) {
        ...
        for (var i=0, j=self.options.ignoreErrors; i<j; i++) {
          if (self.options.ignoreErrors[i] === message) {
            complete();
            return;
          }
        }
        ...
        for (i = 0; i < self.options.ignoreUrls.length; i++) {
    ...
    Raven.arrayMerge = function(arr1, arr2) {
        ...
        Raven.util.each(arr2, function(key, value){
            arr1[key] = value;
        });
        return arr1;
    };
    ...
    Raven.chromeTraceback = function(e) {
        ...
        var chunks, fn, filename, lineno, fileBits,
            traceback = [],
            lines = e.stack.split('\n');
        Raven.util.each(lines.slice(1), function(i, line) {
    ...
    Raven.firefoxOrSafariTraceback = function(e) {
        /*
         * Raven.util.each line is a function with args and a filename, separated by an ampersand.
        ...
        var chunks, fn, args, filename, lineno,
            traceback = [],
            lines = e.stack.split('\n');
        Raven.util.each(lines, function(i, line) {
            if (line) {
                chunks = line.split('@');
                if (chunks[0]) {
                    fn = chunks[0].split('(');
    ...
    Raven.stringifyArguments = function(args) {
        ...
        Raven.util.each(args, function(i, arg) {
            if (arg === undefined) {
    ...
    Raven.send = function(data, complete) {
        var encoded_msg,
        ...
        self.getHeaders(function(headers) {
